...
| GitHub `Staging` environment        | Protected-branch deployments and manual `lapeninns` approval are required. The verified Vercel organisation and project IDs are configured, but no isolated Supabase project or staging credentials exist.                                                                            | Provision a dedicated Nabaperks staging Supabase project, staging-only provider credentials and an independent reviewer before activating promotion.                                      |
| GitHub `Production` environment     | Protected-branch deployments and manual `lapeninns` approval are required. Verified Vercel and Supabase identifiers are configured. `lapeninns` is the repository's only collaborator, so approval is not independent; release credentials are not installed.                         | Add a second trusted collaborator as the independent reviewer, prevent self-review and install dedicated release credentials before activating promotion.                                 |
...
| GitHub `Recovery Drill` environment | Protected-branch deployments and manual `lapeninns` approval are required, with a 30-minute RTO variable. No disposable restored project or restore credentials are configured.                                                                                                       | Add an independent reviewer, restore a physical backup to a new same-region project and retain the successful read-only workflow evidence.                                                |
...
| Vercel environments and protection  | An empty custom `staging` target exists without branch tracking or copied production values. Git integration still deploys `main` automatically. Preview has no shared application configuration, while several historical branches have one-off secret copies.                       | Populate `staging` with isolated credentials, remove obsolete branch-specific Preview copies, and disable automatic `main` deployments only when the build-once GitHub workflow is ready. |
