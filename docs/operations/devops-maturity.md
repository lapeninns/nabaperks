...
| GitHub `Production` environment     | Protected-branch deployments and manual approval are required; self-review is prohibited. Verified Vercel and Supabase identifiers, the Supabase access token and the staged replacement monitor token are configured. `lapeninns` is the repository's only collaborator, so the environment is intentionally fail-closed; the database password, Vercel token and Sentry verification material are not installed.                                                                                                                                               | Add a second trusted collaborator as the independent reviewer, complete the approved database-password rotation with explicit live-service risk acknowledgement, and install dedicated Vercel plus Sentry verification credentials before activating promotion.                                                  |
| GitHub `Monitoring` environment     | The unattended environment exists and permits protected branches only. The public smoke job is source-scoped to this environment and its staged replacement monitor token is configured. The currently deployed application still accepts only the repository-scoped value, so that rollback-safe copy remains until the reviewed dual-secret revision is promoted. Signed external paging secrets are not configured, and the source-owned SLO cannot detect a GitHub-wide monitoring failure independently.                                                    | Promote the reviewed dual-secret revision, prove both protected probes, complete the monitor cutover and remove the repository-scoped copy. Then install the two alert webhook secrets, prove trigger/resolve acknowledgement reaches a human, and add an independent uptime monitor.                            |
...
| Vercel environments and protection  | An empty custom `staging` target exists without branch tracking or copied production values. Git integration still deploys `main` automatically. All 93 obsolete application-secret copies scoped to six merged historical branches have been removed; Preview now contains only the shared Google Maps browser key and therefore cannot run the complete application. Production has the encrypted `PRODUCTION_MONITOR_SECRET_NEXT` overlap value staged, while the database release variables remain untouched pending an explicitly risk-approved live reset. | Populate `staging` and Preview with isolated non-production credentials, promote and prove the dual-secret monitor revision, complete the approved database rotation, then disable automatic `main` deployments only after the build-once GitHub workflow has its protected release credentials and staged path. |
| Sentry production observability     | Sentry server, edge and browser instrumentation, exact-SHA release naming, protected source-map upload, pre-promotion release/project verification and post-promotion deploy readback are source-controlled, but Vercel Production has no Sentry-named variables and GitHub Production has no protected Sentry verification material.                                                                                                                                                                                                                            | Configure `NEXT_PUBLIC_SENTRY_DSN`, `SENTRY_ORG`, `SENTRY_PROJECT` and protected `SENTRY_AUTH_TOKEN` in their required provider scopes, then retain a successful exact-release and deploy readback.                                                                                                              |
| Production revision and DB ledger   | Public liveness reports revision `46ae53a7b920`, while the linked Supabase ledger is missing `20260722100500`, `20260723100000`, and `20260723113000`.                                                                                                                                                                                                                                                                                                                                                                                                           | Apply and verify the reviewed forward-only migrations, then make ledger readback a pre-promotion control so application deployment cannot outrun its schema.                                                                                                                                                     |
...
- The `Production` environment requires an independent reviewer, permits only
  ...
  `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`, `SENTRY_AUTH_TOKEN`,
  `SENTRY_ORG`, `SENTRY_PROJECT` and `PRODUCTION_MONITOR_SECRET` only to
  protected promotion jobs.
...
- Production secrets, Sentry release/source-map settings and cron ownership have
  dated readback without copying secret values into evidence. The protected
  deployment must verify the exact full-SHA Sentry release before promotion and
  read back its production deploy marker afterward.
