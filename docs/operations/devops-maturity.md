...
### Live readback: 22 July 2026

| Surface                             | Observed state                                                                                                                                                                                                                                                                        | Gap to target                                                                                                                                                                                  |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GitHub repository rules             | No ruleset exists. A classic `main` protection rule requires a pull request, one approval, stale-approval dismissal, code-owner review, conversation resolution, linear history and nine individual status checks. It applies to administrators and blocks force pushes and deletion. | Replace the brittle check list with `Release gate` plus CodeQL and dependency review after the new check has run; require approval of the latest reviewable push.                              |
| GitHub `Production` environment     | No required reviewer, wait timer, deployment branch restriction, secret or variable is configured; administrator bypass is enabled.                                                                                                                                                   | Protect production promotion with an independent reviewer, `main`-only deployment and no routine administrator bypass before using it for release credentials.                                 |
| GitHub `Monitoring` environment     | No dedicated environment or signed external paging secrets are configured. The source-owned SLO uses GitHub-hosted smoke history, so it cannot detect a GitHub-wide monitoring failure independently.                                                                                 | Create the `main`-only unattended monitoring environment, install the two alert webhook secrets, prove trigger/resolve acknowledgement reaches a human, and add an independent uptime monitor. |
| GitHub `Recovery Drill` environment | No dedicated protected restore-verification environment or disposable restored project is configured.                                                                                                                                                                                 | Create the reviewer-protected environment, restore a physical backup to a new same-region project and retain the successful read-only workflow evidence.                                       |
| Vercel deployment checks            | Only Vercel `Lint` and `TypeCheck` are configured for Preview and Production.                                                                                                                                                                                                         | Add GitHub `Release gate`, CodeQL and `Database promotion` to Production after this branch is merged and the checks are discoverable.                                                          |
| Vercel environments and protection  | No custom environment is listed. Vercel Authentication is off; protected source maps are on. Git integration still deploys `main` automatically.                                                                                                                                      | Create isolated staging with separate provider credentials and disable automatic `main` deployments when the build-once GitHub workflow is activated.                                          |
| Production revision and DB ledger   | Public liveness reports revision `46ae53a7b920`, while the linked Supabase ledger is missing `20260722100500`.                                                                                                                                                                        | Apply and verify the reviewed forward-only migration, then make ledger readback a pre-promotion control so application deployment cannot outrun its schema.                                    |
