...
| GitHub `Production` environment     | Protected-branch deployments and manual approval are required; self-review is prohibited. Verified Vercel and Supabase identifiers are configured. `lapeninns` is the repository's only collaborator, so the environment is intentionally fail-closed; release and Sentry verification credentials are not installed.                                                  | Add a second trusted collaborator as the independent reviewer and install dedicated release plus Sentry verification credentials before activating promotion.                                                                                     |
| GitHub `Monitoring` environment     | The unattended environment exists and permits protected branches only. The public smoke job is source-scoped to this environment, but its monitor token and signed external paging secrets are not configured. The source-owned SLO uses GitHub-hosted smoke history, so it cannot detect a GitHub-wide monitoring failure independently.                              | Move the monitor token out of repository scope, install the two alert webhook secrets, prove trigger/resolve acknowledgement reaches a human, and add an independent uptime monitor.                                                              |
