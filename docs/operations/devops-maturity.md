...
| Vercel environments and protection  | An empty custom `staging` target exists without branch tracking or copied production values. Git integration still deploys `main` automatically. All 93 obsolete application-secret copies scoped to six merged historical branches have been removed; Preview now contains only the shared Google Maps browser key and therefore cannot run the complete application. Production has only the rotated sensitive `PRODUCTION_MONITOR_SECRET` primary; the temporary `PRODUCTION_MONITOR_SECRET_NEXT` overlap has been removed. Database release variables remain untouched pending an explicitly risk-approved live reset. | Populate `staging` and Preview with isolated non-production credentials, complete the risk-approved database rotation, then add blocking checks and disable automatic `main` deployments only after the build-once GitHub workflow has its protected release credentials and staged path. |
...
| Supabase backups                    | WAL-G and seven completed daily physical backups have no gap above 30 hours; the latest completed backup was 8.3 hours old at readback. Point-in-time recovery is disabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                | Enable PITR, approve RPO/RTO and complete a non-production restore drill before claiming recovery readiness.                                                                                                                                                                              |
