...
| 32 | Public QR router `/q/[qrId]` | Resolves QR, rate-limits scans, records scan event, redirects existing members to stamp or new customers to join. | Source-contract coverage now pins server-side QR resolution, rate-limited and unavailable states, existing/new member branching, and encoded QR redirect values; Playwright coverage pins the invalid-QR unavailable state on mobile and desktop without CSP errors or overflow. Availability and billing logic must stay aligned with launch QR readiness, and QR scan events can become noisy under bot/scanner traffic. | Add live DB/browser QR resolver tests for inactive, unavailable merchant, billing-blocked, existing member, new member, and rate-limited cases. |
| 33 | Customer join/OTP/membership | Join wizard uses phone OTP, signed customer session cookies, terms acceptance, and membership/first-stamp RPCs. | OTP anti-enumeration and first-stamp qualification depend on QR context and session state being handled precisely. Join redirects now encode QR form state before writing it back into URLs. | Add full QR-to-join E2E plus action tests for unknown phone, wrong OTP, expired OTP, terms missing, no QR, existing membership. |
...
| 38 | Reward QR image | Route mints or renders short-lived reward scan token QR only when reward/customer/availability gates pass. | Expired scan-token cleanup and safe token reuse are now defined in SQL. | Apply migration in target Supabase and add owner/expired/redeemed/blocked/not-ready integration tests. |
