...
| 33 | Customer join/OTP/membership | Join wizard uses phone OTP, signed customer session cookies, terms acceptance, and membership/first-stamp RPCs. | OTP anti-enumeration and first-stamp qualification depend on QR context and session state being handled precisely. Join redirects and OTP fallback links now encode QR form state before writing it back into URLs. Local live-DB browser proof covers QR scan to phone OTP, wrong-code feedback without membership creation, terms acceptance, card redirect, membership creation, first-stamp issue, and join event recording. | Add action/browser tests for unknown phone, expired OTP, missing terms, no QR, and existing membership. |
