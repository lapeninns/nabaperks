...
| 41  | Customer dashboard `/home`               | Authenticated dashboard reads cards, rewards, top redeemable item, and recent activity.                    | Local live-DB browser proof now covers current-customer empty wallet, one-card ready/redeemable wallet, waiting reward wallet, redeemed activity readback, raw PII exclusion, and no horizontal overflow.             | Keep customer-owned dashboard fixture coverage in the customer-flow gate; replay against staging/provider seed data before release.                              |
...
- Session validation and the customer identity read are one service-role RPC
  (`touch_customer_session_and_load`), memoised per request, so the
  device-bound touch happens exactly once however many loaders ask.
