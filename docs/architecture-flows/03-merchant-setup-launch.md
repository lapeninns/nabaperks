...
| 22  | QR image rendering             | Route handler renders merchant-owned QR images from QR code id/context.                                                                                       | Owned image/poster context now requires the authenticated merchant, primary location, active card, join destination type, and active QR before image bytes can render.                                                                                                           | Add authenticated fixture coverage for wrong-merchant, missing, inactive, and valid image paths.                                                                         |
