...
| 19  | Reward pool setup              | Server actions upsert, toggle, delete/archive reward pool items and can trigger QR provisioning after enough active rewards.                                  | App and SQL QR guards now share the three-active-reward minimum, and live-DB rollback coverage now executes the merchant-owned reward pool RPC lifecycle, including QR threshold creation and disabled QR re-enable.                                                               | Re-run the reward pool lifecycle proof on target/staging after migrations and seed/session parity are available; add support visibility for reward edits if operators need it. |
...
- Venue server actions and target/staging reward pool replay.
...
P1 source drift is remediated for the QR reward threshold, QR provisioning has
been moved out of GET render, and the reward-pool RPC lifecycle now has local
live-DB proof. Before treating launch as pilot-ready, apply the migration and
run target/provider proof for the QR, billing, and customer entry paths.
