...
| --- | --- | --- | --- | --- |
| 26 | Customer/member readback `/app/customers` | Session merchant loads masked member rows and total count; client table owns search/filter over masked DTOs. | The scan-token CTA mismatch has been removed; readback now sends merchants to the scanner instead of linking reward event ids. DTO unit coverage proves raw email/phone and server-only reward/customer internals stay out of client rows, and local live-DB browser proof now covers signed-merchant masked readback, highlight state, count display, scanner CTA, raw-email search exclusion, and no overflow. | Keep the signed-merchant readback fixture in the local gate; add capped-row and empty-state browser cases when support-scale row volumes need them. |
...
- Capped-row and empty-state member-table browser proof when support-scale row volumes need it.
...
support-scale capped/empty member-table proof if row volumes make it necessary.
