...
> **Note:** This is the durable narrative of the offer. The authoritative copy lives in `lib/marketing/facts.ts` (`OFFER`, `MARKET`, `PLAN_LINE`, `DFY_LAUNCH`, `CORE_OFFER`, `BONUS_STACK`, `GUARANTEE`, `GUARANTEE_ROI`, `SCARCITY`, `PROMO`); if the two ever disagree, the code is right and this doc should be updated.

> **Offer v3 (18 July 2026, amended 19 July).** The commercial model was re-locked from the owner's finalised pack in `Offers- Nabaperks-Finalized/` and rebuilt into the public site: the primary wrapper is now **"The 30-Day Gastropub Mid-Week Revenue Accelerator"** (this doc's "First-Regular Launch" survives as the ASA-safer alternate, `OFFER.nameSafe`), delivery is a **done-for-you launch**, the guarantee stack adds the **90-Day ROI Extension**, and honest scarcity is the real **5 DFY onboardings/week** cap. **The setup fee has been removed entirely (owner decision 19 July) — the offer is a pure subscription (`PLAN_LINE`) with the launch included; the pack's £99 setup no longer applies.** The narrative below predates v3; where it disagrees with `lib/marketing/facts.ts`, the code is right.
