...
| 03#46 | Medium   | [ ]         | BLOCKED BY TEST: reverted. merchant-onboarding-continuity requires the first invalid field to be focused after a failed submit; the blur-validation restructure breaks it and I could not reproduce the failure outside the suite. The `SubmitButton` half was STALE — already adopted. Added blur-time validation for the five required fields, change-time clearing, and a visible `role="alert"` summary of every invalid field as 44px anchor links. Postcode/city auto-formatting deliberately left: that is server-side normalisation, not UI. |
