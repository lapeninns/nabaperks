...
done (rest blocked, see NEEDS-SIGNOFF.md), `[ ]` outstanding.

## 05-design-system.md — 25 done / 1 partial / 41 open (of 67)
...
| [ ] | 05#5  | High     | Sun (`--seal`) is unusable as a foreground yet is exposed as `text-sun`                  |
| [ ] | 05#6  | Medium   | `.eyebrow` bakes in a colour, so it cannot be used on the inverted band                  |
...
| [x] | 05#18 | High     | `Card`'s stock 24px radius survives on nested images and its `ring-1` survives everywher |
...
| [x] | 05#35 | Medium   | Sheet overlay fades in 150ms while the sheet slides 320ms; and its `shadow-xl` is meanin |
...
| [ ] | 05#44 | Medium   | `AdminShell` sidebar footer stacks four `MonoTag` chips of pure decoration               |
| [ ] | 05#45 | Low      | `PageTitle` fakes baseline alignment with `md:pt-8`                                      |
| [ ] | 05#46 | Medium   | `CustomerAppShell` reserves 128px of bottom padding for a 56px tab bar                   |
| [ ] | 05#47 | High     | The marketing footer is a 4-column, 13-link, 44px-per-row block on every page            |
| [ ] | 05#48 | High     | No error boundary moves focus or announces itself                                        |
| [ ] | 05#49 | Medium   | `global-error.tsx` speaks a different design system                                      |
| [ ] | 05#50 | Medium   | Loading fallbacks use `role="status"` on a container with no `aria-live` guarantee and n |
...
| [ ] | 05#59 | Low      | `WetInkWiggle` / `WetInkBreathe` are documented as one-shot but read as loops in the tok |
| [x] | 05#60 | Medium   | The global reduced-motion rule nukes `transition-duration` on _everything_, including pr |
...
| [ ] | 05#65 | Medium   | `Section` and `ContrastBand` own marketing rhythm, but nothing owns console/customer rhy |
...
## 01-marketing.md / 02-customer.md / 03-merchant.md / 04-admin.md

Not yet started in this pass, beyond the cross-cutting fixes already shipped
(04#1, 04#19, 04#29, 04#33, 04#41, 04#46, 04#48, 04#59, 03#25, 03#28, 03#29,
03#48, 02#40, 01#12, 01#63). Tracked in the individual reports.
