...
## 55. The button's radius was dead code on `<Button>` and live on the one call site that does not use `<Button>` — and that call site still ships without a border
...
## 56. DESIGN.md's named circle-exception list is stale in **both** directions
...
## 57. There is no on-ink dashed tone in DESIGN.md, and one surface invented one
...
## 58. The customer join stepper's bars contradict the Progress clause on four axes at once
...
## 59. `StampDot`'s earned initials are a third size in a two-size register, and every gate agrees with it
...
## 60. Sweep residue — five things measured, none of them this lane's to decide
