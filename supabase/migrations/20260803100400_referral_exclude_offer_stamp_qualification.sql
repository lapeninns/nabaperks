-- Referral qualification must ignore promotional offer-campaign stamps.
--
-- claim_offer_campaign (20260803100200) enrols a new member from a printed
-- campaign QR and writes the campaign's bonus stamps with metadata source
-- 'offer_campaign' and a NULL business date — a joining incentive, not a
-- genuine venue visit. qualify_referral_on_stamp already excludes
-- referral_bonus / imported / manual_adjustment / loyalty_invite from being a
-- qualifying visit; offer_campaign belongs in exactly the same set. Without
-- this, a customer who joined through an offer would settle a referral bonus
-- for their referrer the moment they claimed, before ever visiting the venue.
--
-- Verbatim from 20260723100000_referral_exclude_invite_stamp_qualification.sql
-- with 'offer_campaign' added to the excluded-source list; signature and grants
-- are unchanged, so create-or-replace preserves the existing ACL.
