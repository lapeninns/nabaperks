...
alter table public.auth_hook_deliveries
  ...
  add column if not exists lease_api_version smallint,
...
create or replace function public.claim_auth_hook_delivery_v2(
  p_channel text,
  p_webhook_id text
)
returns jsonb
...
begin
  ...
  insert into public.auth_hook_deliveries (
    channel, webhook_id, status, lease_id, lease_expires_at, lease_api_version,
  ...
  if v_row.status = 'processing' then
    ...
    if v_row.provider_attempted_at is not null then
      return jsonb_build_object('status', 'replay');
    end if;
  end if;
...
create or replace function public.mark_auth_hook_delivery_attempted_v2(
  p_channel text,
  p_webhook_id text,
  p_lease_id uuid
)
...
begin
  ...
  update public.auth_hook_deliveries
  set provider_attempted_at = coalesce(provider_attempted_at, clock_timestamp()),
      updated_at = clock_timestamp()
  where channel = p_channel
    and webhook_id = nullif(btrim(p_webhook_id), '')
    and status = 'processing'
    and lease_api_version = 2
...
create or replace function public.complete_auth_hook_delivery_v2(
  p_channel text,
  p_webhook_id text,
  p_lease_id uuid
)
...
create or replace function public.fail_auth_hook_delivery_v2(
  p_channel text,
  p_webhook_id text,
  p_lease_id uuid
)
...
-- Preserve the committed JSON/lease wire contract during a rolling deploy,
-- but route old application instances through the v2 state machine. The old
-- app cannot distinguish a definitive rejection from response loss, so its
-- failure callback seals the exact lease rather than reopening a possible send.
...
set search_path = public
as $function$
  select public.claim_auth_hook_delivery_v2(p_channel, p_webhook_id);
$function$;
...
as $function$
  select public.complete_auth_hook_delivery_v2(
    p_channel, p_webhook_id, p_lease_id
  );
$function$;
...
begin
  ...
  update public.auth_hook_deliveries
  set status = 'completed',
      provider_attempted_at = coalesce(provider_attempted_at, clock_timestamp()),
      completed_at = clock_timestamp(),
      lease_expires_at = null,
      updated_at = clock_timestamp()
  where channel = p_channel
...
revoke all on function public.claim_auth_hook_delivery_v2(text, text) from public, anon, authenticated;
revoke all on function public.mark_auth_hook_delivery_attempted_v2(text, text, uuid) from public, anon, authenticated;
revoke all on function public.complete_auth_hook_delivery_v2(text, text, uuid) from public, anon, authenticated;
revoke all on function public.fail_auth_hook_delivery_v2(text, text, uuid) from public, anon, authenticated;
...
grant execute on function public.claim_auth_hook_delivery_v2(text, text) to service_role;
grant execute on function public.mark_auth_hook_delivery_attempted_v2(text, text, uuid) to service_role;
grant execute on function public.complete_auth_hook_delivery_v2(text, text, uuid) to service_role;
grant execute on function public.fail_auth_hook_delivery_v2(text, text, uuid) to service_role;
