...
begin
  ...
  v_contact := concat_ws(
    ' · ',
    nullif(btrim(v_merchant_email), ''),
    nullif(btrim(v_merchant_phone), '')
  );
  ...
  new.terms_snapshot := jsonb_build_object(
    ...
    'sections', jsonb_build_array(
      ...
      jsonb_build_object(
        'id', 'merchant-contact',
        'body', coalesce(nullif(v_contact, ''), 'Ask the venue team')
      )
    )
  );
...
revoke all on function public.apply_customer_legal_terms_snapshot_v20260715()
  from public, anon, authenticated;
grant execute on function public.apply_customer_legal_terms_snapshot_v20260715()
  to service_role;
