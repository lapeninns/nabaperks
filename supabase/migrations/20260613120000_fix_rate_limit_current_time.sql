-- enforce_rate_limit declared a variable named current_time, but CURRENT_TIME
-- is a reserved SQL keyword (timetz), so `reset_at <= current_time` parsed as
-- a timestamptz/timetz comparison and raised on every call that found an
-- existing bucket. First calls inserted and returned early, which is why the
-- bug only surfaced on repeat calls inside a window.
