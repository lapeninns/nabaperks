-- First-cycle reward should be the default advertised reward, not a surprise.
--
-- Keep weighted randomness for later cycles, but pin cycle 1 unlocks to the
-- first active reward pool item (display_order asc, created_at asc, id asc).
-- This is idempotent via create or replace.
