-- A revoked subscription is historical device state, even when the current
-- session belongs to the same customer. Require the prior browser keys before
-- reactivating it; active same-customer registration remains idempotent.
...
begin
  ...
  if existing_owner.id is not null
     and (
       existing_owner.customer_id <> p_customer_id
       or not existing_owner.enabled
       or existing_owner.revoked_at is not null
     )
     and (
