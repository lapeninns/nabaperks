-- Align newly accepted customer and venue terms with account-level verified
-- email collection. Historical acceptance snapshots remain immutable.

create or replace function public.apply_customer_legal_terms_snapshot_v20260719()
returns trigger
language plpgsql
security definer
...
begin
  if new.policy_version <> '2026-07-19' then
    return new;
  end if;
...
revoke all on function public.apply_customer_legal_terms_snapshot_v20260719()
  from public, anon, authenticated;
grant execute on function public.apply_customer_legal_terms_snapshot_v20260719()
  to service_role;

drop trigger if exists customer_terms_apply_v20260715_snapshot
  on public.customer_loyalty_terms_acceptances;
create trigger customer_terms_apply_v20260715_snapshot
  before insert on public.customer_loyalty_terms_acceptances
  for each row
  execute function public.apply_customer_legal_terms_snapshot_v20260715();

drop trigger if exists customer_terms_apply_v20260719_snapshot
  on public.customer_loyalty_terms_acceptances;
create trigger customer_terms_apply_v20260719_snapshot
  before insert on public.customer_loyalty_terms_acceptances
  for each row
  execute function public.apply_customer_legal_terms_snapshot_v20260719();
