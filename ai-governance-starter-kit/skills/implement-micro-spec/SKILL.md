...
3. **Record proof as you go:**
   `<pkg> governance:run-gates --spec MS-<area>-<slug> --record`
   writes the run into the spec's evidence ledger — including red runs, which
   are honest history, not something to delete.
4. **Amend the spec first** when reality diverges: adjust requirements,
