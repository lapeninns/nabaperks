...
import { assertDisposableDbTarget } from "./disposable-db-target.mjs"
...
export async function checkLocalRaceTarget(env) {
  ...
  console.log(
    `Authenticated loopback race targets are current for ${project.projectId}.`
  )
}
...
function requireLoopbackUrl(value, name) {
  ...
  try {
    ...
    assert.fail(`${name} must be an absolute loopback URL.`)
  }

  assert.equal(url.protocol, "http:", `${name} must use IPv4 loopback.`)
...
async function probeAuthenticatedTarget(url, authToken, name) {
  ...
  assert.ok(response.ok, `${name} rejected the authenticated probe.`)
}
