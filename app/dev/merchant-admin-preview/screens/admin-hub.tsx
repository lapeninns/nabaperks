...
import { ADMIN_FUNNEL_COUNTS, ADMIN_OVERVIEW } from "./mock-data"

/**
 * Mirror of `/admin` (the admin hub). Reuses the real `MetricTile`, `FunnelChart`,
 * `AdminPanel`, and `SourceLabel` with mock overview counts. The live page reads
 * these from service-role admin readbacks and product events.
 */
  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        <MetricTile
          label="Merchants"
          value={ADMIN_OVERVIEW.merchants}
          icon={Store01Icon}
        />
        <MetricTile
          label="Customers"
          value={ADMIN_OVERVIEW.customers}
          icon={UserMultiple02Icon}
        />
        <MetricTile
          label="Billing issues"
          value={ADMIN_OVERVIEW.billingIssues}
          icon={CreditCardIcon}
        />
      </section>
      ...
      <AdminPanel>
        ...
        />
        <FunnelChart
          items={Object.entries(ADMIN_FUNNEL_COUNTS).map(([label, value]) => ({
            label,
            value,
          }))}
        />
