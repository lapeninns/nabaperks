...
export function SampleLoyaltyCard({
  ...
  return (
    <div
      className={cn(
        "mx-auto w-full max-w-[min(100%,21rem)] [filter:drop-shadow(3px_3px_0_var(--w-shadow-color))] sm:[filter:drop-shadow(4px_4px_0_var(--w-shadow-color))]",
        className
      )}
    >
      <div
        className={cn(
          "rounded-t-[var(--radius)] border-2 border-b-0 border-ink bg-card px-3.5 pt-4 pb-3.5 sm:px-5 sm:pt-5 sm:pb-4",
          shellClassName
        )}
      >
        <div className="flex items-start justify-between gap-2.5 sm:gap-3">
          <div className="min-w-0 flex-1">
            ...
            <div
              className={cn(
                "mt-1 text-base leading-tight font-extrabold text-balance sm:mt-1.5 sm:text-xl",
                titleBlockClassName
              )}
            >
              {title}
          ...
          <VenueMark
            initials={venueInitials}
            size={50}
            className="shrink-0 origin-top-right scale-[0.88] sm:scale-100"
          />
        </div>
        ...
        {showFooter ? (
          <>
            ...
            <div className="mono-id flex items-center justify-between gap-2 font-normal sm:gap-3">
              <span className="min-w-0 truncate text-muted-foreground">
                {footerLeft}
              </span>
              <span className="min-w-0 shrink-0 text-right sm:min-w-[9.75rem]">
                {footerRight}
              </span>
            </div>
          </>
