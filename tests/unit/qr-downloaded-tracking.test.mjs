...
test("poster print builds exactly the registered qr_downloaded event", () => {
  ...
  assert.equal(event.eventName, "qr_downloaded", "the registered event name is used")
  assert.equal(event.merchantId, MERCHANT_ID, "the event is attributed to the acting merchant")
...
test("the activity-feed metadata contract is satisfied", () => {
  ...
  assert.equal(event.metadata.asset_type, "poster_pdf", "poster prints map to poster_pdf")
  assert.equal(event.metadata.template, "thermal", "the template id is preserved for analysis")
})
...
test("a poster without a resolvable QR still records with null qr_code_id", () => {
  ...
  assert.equal(event.qrCodeId, null, "qr_code_id is null, not undefined or fabricated")
