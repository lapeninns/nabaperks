...
test("Given global requests When proxy responds Then all security header families are attached", () => {
  ...
  assert.match(proxy, /COMMON_SECURITY_HEADERS/)
  assert.match(nextConfig, /COMMON_SECURITY_HEADERS/)
  assert.match(csp, /Strict-Transport-Security/)
  assert.match(csp, /X-Frame-Options[^\n]+DENY/)
  assert.match(csp, /X-Content-Type-Options[^\n]+nosniff/)
  assert.match(csp, /Referrer-Policy[^\n]+strict-origin-when-cross-origin/)
  assert.match(csp, /Permissions-Policy/)
  assert.match(csp, /camera=\(self\)/)
  assert.match(csp, /microphone=\(\)/)
  ...
  assert.doesNotMatch(proxy, /isStaticMarketingPath/)
