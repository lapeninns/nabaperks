...
test("Given the self-stamp action receives form data When source is inspected Then QR proof is validated before stamping", () => {
  ...
  assert.match(action, /if \(!membershipId \|\| !qrId\) \{[\s\S]*Scan the venue code to add your stamp/)
  assert.match(action, /getStampQrContextForMembership\(membershipId, qrId\)/)
  assert.match(action, /if \(!qrContext\) \{[\s\S]*Scan the venue code to add your stamp/)
  assert.match(action, /issueSelfServiceStamp\(membershipId, coordinates\(formData\)\)/)
  assert.ok(
    action.indexOf("getStampQrContextForMembership(membershipId, qrId)") <
      action.indexOf("issueSelfServiceStamp(membershipId, coordinates(formData))"),
...
test("Given a QR id crosses into stamping When context is resolved Then inactive, unavailable, and wrong-membership QR rows are rejected", () => {
  ...
  assert.match(resolver, /if \(!qrContext \|\| !qrContext\.available\) return null/)
  assert.match(
    resolver,
    /getExistingMembershipForCurrentUser\(\s*qrContext\.merchant\.id\s*\)/
  )
  assert.match(resolver, /if \(!membership \|\| membership\.id !== membershipId\) return null/)
})
...
test("Given stamp context is loaded for the page When QR is missing or invalid Then the UI receives non-stamping state", () => {
  ...
  assert.match(loader, /if \(!qr\) \{[\s\S]*qrValid: false[\s\S]*qrMissing: true/)
  assert.match(loader, /const qrContext = await getStampQrContextForMembership\(membershipId, qr\)/)
  assert.match(loader, /if \(!qrContext\) \{[\s\S]*qrValid: false[\s\S]*qrMissing: false/)
  assert.ok(
...
test("Given a stamp is issued When post-issue side effects run Then they occur only after the RPC succeeds", () => {
  ...
  assert.match(action, /if \(result\.status === "blocked"\) \{[\s\S]*return fail\(result\.reason\)/)
  assert.ok(
  ...
  assert.match(action, /enqueueStampTransitionNotifications\(\{[\s\S]*rewardUnlocked: result\.rewardUnlocked/)
})
