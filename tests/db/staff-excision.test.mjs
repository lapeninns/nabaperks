...
async function seededOwner(tx, merchantId) {
  ...
  assert.ok(
    row?.owner_user_id,
    `merchant ${merchantId} must have a seeded owner`
  )
...
test(
  "the two staff-named policies are renamed, the seven scoped ones survive",
  { skip },
  async () => {
    const sql = db()
    ...
    assert.ok(
      names.includes("qr_codes_select_owner_admin"),
      "qr_codes policy renamed"
    )
    assert.ok(
    ...
    assert.ok(
      !names.includes("qr_codes_select_owner_staff_admin"),
      "old qr_codes name gone"
    )
    assert.ok(
    ...
    for (const scoped of [
      ...
      assert.ok(
        names.includes(scoped),
        `${scoped} still exists under its own name`
      )
    }
  }
)
...
test(
  "owner and admin access is unchanged; other tenants stay locked out",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      ...
      const ownQr = await asAuthenticatedUser(
        tx,
        OWNER_A,
        (sp) =>
      ...
      const ownPool = await asAuthenticatedUser(
        tx,
        OWNER_A,
        (sp) =>
      ...
      const adminQr = await asAuthenticatedUser(
        tx,
        ADMIN,
        (sp) =>
          sp`select id from public.qr_codes where merchant_id = ${MERCHANT_A}::uuid`
      ...
      assert.ok(
        adminQr.length >= 1,
        "the internal admin still sees merchant QR codes"
      )

      const foreignQr = await asAuthenticatedUser(
        tx,
        OWNER_B,
        (sp) =>
          sp`select id from public.qr_codes where merchant_id = ${MERCHANT_A}::uuid`
      ...
      assert.equal(
        foreignQr.length,
        0,
        "another tenant's owner sees none of merchant A's QR codes"
      )

      const foreignPool = await asAuthenticatedUser(
        tx,
        OWNER_B,
        (sp) =>
          sp`select id from public.reward_pool_items where merchant_id = ${MERCHANT_A}::uuid`
      ...
      assert.equal(
        foreignPool.length,
        0,
        "another tenant's owner sees none of merchant A's pool"
      )

      const foreignMemberships = await asAuthenticatedUser(
        tx,
        OWNER_B,
        (sp) =>
...
async function asAuthenticatedUser(tx, userId, fn) {
  return tx.savepoint(async (sp) => {
    ...
    // Internal-admin authority requires a current AAL2 session.
