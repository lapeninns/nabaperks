...
test("a double redeem is idempotent and never advances the cycle twice", { skip }, async () => {
  await inRolledBackTxn(async (tx) => {
    ...
    const before = (await tx`
      ...
      from public.customer_memberships where id = ${m.membership_id}`)[0]
    ...
    const afterFirst = (await tx`
      select active_cycle_number, total_rewards_redeemed
      from public.customer_memberships where id = ${m.membership_id}`)[0]
    assert.equal(afterFirst.active_cycle_number, before.active_cycle_number + 1, "cycle advances once")
    assert.equal(
...
test("a token can only be collected by the merchant it belongs to", { skip }, async () => {
  await inRolledBackTxn(async (tx) => {
    ...
    assert.ok(other, "a second merchant exists to attempt cross-tenant collection")
    ...
    try {
      ...
      rejected = /different merchant|not found|belongs/i.test(String(error.message))
    }
    assert.ok(rejected, "a foreign merchant cannot collect another venue's token")
    const [{ consumed_at }] = await tx`
      select consumed_at from public.reward_scan_tokens where id = ${minted.scan_token}`
    assert.equal(consumed_at, null, "the token stays unconsumed after a cross-tenant attempt")
  })
})
