...
test("no schema object references plaintext customers.phone", { skip }, async () => {
  const sql = db()
  ...
  assert.ok(!/customers\.phone[^_]/.test(viewDef), "the masked view has no plaintext fallback")
  assert.match(viewDef, /phone_last4/, "the masked view serves from the stored last4")

  const [{ def: checkDef }] = await sql.unsafe(
    `select pg_get_constraintdef(oid) as def from pg_constraint
     where conname='customers_contact_present'`
  )
  assert.match(checkDef, /phone_last4/, "the contact CHECK accepts the backfilled last4")
  assert.ok(!/phone IS NOT NULL/.test(checkDef.replace(/phone_(hmac|last4)/g, "")), "the contact CHECK no longer references plaintext phone")
})

test("the contact CHECK accepts last4-only rows and rejects contact-less rows", { skip }, async () => {
  await inRolledBackTxn(async (tx) => {
    const [legacy] = await tx`
      insert into public.customers (phone_last4, created_at, updated_at)
      values ('4321', now(), now())
      returning id`
    assert.ok(legacy.id, "a backfilled legacy row (last4 only) passes the CHECK")
...
test("the masked view serves Phone ending NNNN from last4", { skip }, async () => {
  await inRolledBackTxn(async (tx) => {
    ...
    assert.equal(rows[0].phone, "Phone ending 9876", "the masked phone derives from last4")
    assert.equal(rows[0].phone_last4, "9876")
  })
})

test("the verified-contact trigger still guards the encrypted set", { skip }, async () => {
  await inRolledBackTxn(async (tx) => {
    const [customer] = await tx`
    ...
    try {
      ...
      refused = /Verified customer phone cannot be changed/i.test(String(error.message))
    }
    assert.ok(refused, "verified phone identity stays immutable after the column drop")
  })
})

test("get_reward_scan_context answers without a raw phone column", { skip }, async () => {
  await inRolledBackTxn(async (tx) => {
    const [row] = await tx`
    ...
    assert.ok(!("customer_phone" in row), "no raw phone column remains in the result")
    assert.ok("customer_phone_last4" in row, "the masked last4 column remains")
  })
})
