...
test(
  "lifecycle: create → attach for a member mints the reward and scrubs the hashes",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const fixture = await createRewardPoolFixture(tx)
...
test(
  "a matched non-member stays matched (attaches later) with hashes intact",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const fixture = await createRewardPoolFixture(tx)
...
test(
  "dedupe: a second invite for the same contact returns the first",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const fixture = await createRewardPoolFixture(tx)
...
test(
  "create refuses billing-blocked merchants before storing an invite",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const fixture = await createRewardPoolFixture(tx)
...
test(
  "attach is idempotent and billing-blocked members do not attach",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const fixture = await createRewardPoolFixture(tx)
...
test(
  "attach honours the direct reward same-day member cap",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const fixture = await createRewardPoolFixture(tx)
      ...
      assert.equal(
        attached.length,
        0,
        "the invite stays matched when the member cap is full"
      )
...
test(
  "sticky match: a token never re-targets an invite matched to someone else",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const other = await createRewardPoolFixture(tx)
      const tokenHash = hex64()
      const invitedPhone = hex64()
      const invite = await createInvite(tx, other.merchantId, {
        phoneHmac: invitedPhone,
        tokenHash,
      })
      ...
      // A is the invited contact. Since 20260801140000 the claim token only
      // NARROWS the search — it is no longer an alternative to the contact match
      // — so the invited phone HMAC has to be presented too.
      await attach(tx, a, { phone: invitedPhone, token: tokenHash })
      let [row] = await tx`
      ...
      // B follows the same token with the same contact — sticky: the first
      // match holds.
      await attach(tx, b, { phone: invitedPhone, token: tokenHash })
      ;[row] = await tx`
...
test(
  "a leaked token alone cannot reserve an invite for a stranger",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const other = await createRewardPoolFixture(tx)
      ...
      // A stranger presents ONLY the leaked claim URL. Before 20260801140000
      // this reserved the invite against them, permanently denying the genuine
      // recipient — a denial-of-service primitive on top of the theft.
      const stranger = await bareCustomer(tx)
      await attach(tx, stranger, { token: tokenHash })

      const [row] = await tx`
      select status, matched_customer_id from public.pending_reward_invites where id = ${invite.invite_id}::uuid`
      assert.equal(row.status, "pending")
      assert.equal(row.matched_customer_id, null)
    })
  }
)

test(
  "expire_and_purge expires + scrubs live invites and hard-deletes old terminal ones",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const fixture = await createRewardPoolFixture(tx)
      ...
      const [{ count }] =
        await tx`select public.expire_and_purge_reward_invites(now()) as count`
      assert.ok(count >= 1)
      const [row] = await tx`
...
test(
  "RLS: only internal admins can read invites; the audit carries no raw contact",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const fixture = await createRewardPoolFixture(tx)
...
test(
  "erasure cancels + scrubs a customer's matched invites",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const fixture = await createRewardPoolFixture(tx)
      ...
      assert.equal(
        row.status,
        "cancelled",
        "a matched invite is cancelled on erasure"
      )
...
test(
  "erasure cancels + scrubs unmatched email-keyed invites through customer email_hmac",
  { skip },
  async () => {
    await inRolledBackTxn(async (tx) => {
      const fixture = await createRewardPoolFixture(tx)
      ...
      assert.equal(
        row.status,
        "cancelled",
        "email-keyed invite is cancelled on erasure"
      )
