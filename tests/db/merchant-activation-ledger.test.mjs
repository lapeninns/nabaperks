...
test("activation schema pins occurrence time, idempotency, FORCE RLS, and service-only RPCs", { skip }, async () => {
  assert.ok(
  ...
  await inRolledBackTxn(async (tx) => {
    ...
    for (const fn of functions) {
      ...
      assert.equal(fn.service_execute, true, `${fn.proname}: service role executes`)
...
test("valid account attribution links once while malformed and conflicting links fail closed", { skip }, async () => {
  await inRolledBackTxn(async (tx) => {
    ...
    await insertAccountEvent(tx, ownerOne, funnelKey, new Date().toISOString())
    await insertAccountEvent(tx, ownerTwo, funnelKey, new Date().toISOString())
    await tx`
    ...
    assert.deepEqual(links, [{ owner_user_id: ownerOne, funnel_key: funnelKey }])
  })
})

test("activation recorder is closed-vocabulary, timestamp-bounded, and retry-idempotent", { skip }, async () => {
  await inRolledBackTxn(async (tx) => {
    const fixture = await createMerchantFixture(tx, "recorder")
    const occurredAt = new Date(Date.now() - 60_000).toISOString()

    const first = await asServiceRole(tx, (sp) =>
    ...
    const replay = await asServiceRole(tx, (sp) =>
      sp`select public.record_merchant_activation_event(
        ${fixture.merchantId}::uuid,
        'merchant_launch_entered',
        'first-entry',
...
test("cohort aggregation stays exact beyond the PostgREST row cap and returns aggregate facts only", { skip }, async () => {
  await inRolledBackTxn(async (tx) => {
    const cohortStart = new Date(Date.now() - 24 * 60 * 60 * 1_000)
...
test("authoritative stages, stamp exclusions, and seven-day outcomes are derived without double counting", { skip }, async () => {
  await inRolledBackTxn(async (tx) => {
    const asOf = new Date("2026-07-10T12:00:00.000Z")
    const cohortStart = new Date("2026-06-20T00:00:00.000Z")
    const cohortEnd = new Date("2026-07-11T00:00:00.000Z")

    const yes = await seedActivationJourney(tx, {
      slug: "yes",
      accountAt: "2026-07-01T09:00:00.000Z",
      billingAt: "2026-07-02T09:00:00.000Z",
      stampAt: "2026-07-03T09:00:00.000Z",
      setup: true,
    })
    await seedActivationJourney(tx, {
      slug: "no",
      accountAt: "2026-06-25T09:00:00.000Z",
      billingAt: "2026-06-26T09:00:00.000Z",
      stampAt: null,
      setup: false,
    })
    await seedActivationJourney(tx, {
      slug: "pending",
      accountAt: "2026-07-09T08:00:00.000Z",
      billingAt: "2026-07-09T09:00:00.000Z",
      stampAt: null,
      setup: false,
    })
...
test("merchant activation migration replays without changing its contract", { skip }, async () => {
...
async function insertAccountEvent(tx, ownerId, funnelKey, occurredAt) {
  await tx`
    insert into public.product_events (
      ...
      jsonb_build_object('funnel_key', ${funnelKey}),
      ${occurredAt}::timestamptz, ${occurredAt}::timestamptz
    )`
}

async function createMerchantFixture(tx, slug, createdAt = new Date().toISOString()) {
...
async function seedActivationJourney(tx, options) {
  const fixture = await createMerchantFixture(tx, options.slug, new Date(new Date(options.accountAt).getTime() + 2 * 60 * 60 * 1_000).toISOString())
  await insertAccountEvent(tx, fixture.ownerId, options.slug[0].repeat(64), options.accountAt)
  ...
  const at = (hours) =>
    new Date(new Date(options.accountAt).getTime() + hours * 60 * 60 * 1_000).toISOString()
  ...
  const customer = await createStampFixture(tx, fixture, locationId, cardId, "real")
  await tx`
...
async function seedExcludedStampRows(tx, fixture) {
  const cases = [
    ["referral", "earned", 1, "referral_bonus", "2026-06-28T09:00:00.000Z"],
    ["manual", "manual_adjustment", 2, "self_service_qr", "2026-06-29T09:00:00.000Z"],
    ["reversed", "reversed", -1, "self_service_qr", "2026-06-30T09:00:00.000Z"],
    ["other", "earned", 1, "merchant_manual", "2026-07-01T10:00:00.000Z"],
  ]
  for (const [slug, eventType, delta, source, createdAt] of cases) {
    ...
    await tx`
      insert into public.stamp_events (
        ...
        jsonb_build_object('source', ${source}), ${createdAt}::timestamptz
      )`
  }
}
