...
describe("MS-merchant-reward-presets source contract", () => {
  it("passes preset catalogues from launch panels into the existing forms", () => {
    const cardPanel = readProjectFile("components/merchant/launch/card-panel.tsx")
    ...
    assert.match(rewardsPanel, /PUB_REWARD_PRESETS/)
    assert.match(rewardsPanel, /presets=\{PUB_REWARD_PRESETS\}/)
  })
