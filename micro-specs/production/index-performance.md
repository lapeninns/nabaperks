...
allowed_blast_radius:
  ...
  - .lighthouserc.json
  - tests/e2e/production-review-closure.desktop.spec.ts
...
The admin layout metadata, response-level indexing header, Lighthouse
configuration, CI release job, one focused browser test, and this Micro-Spec
may change. Admin authorization, page content, unrelated visual baselines, and
application business logic are out of scope.
...
- **IP-1:** WHEN any `/admin` route responds or redirects, THE response SHALL prohibit indexing and following; rendered admin layouts SHALL emit the same metadata.
- **IP-2:** WHEN Lighthouse evaluates the public launch URLs, THE collector SHALL use mobile form-factor emulation for at least two runs.
- **IP-3:** IF any declared Lighthouse category or timing budget fails, THEN THE pull-request CI job SHALL fail.

## 6. Verification Criteria and Task Breakdown

1. Add focused assertions for `/admin` response and layout robots contracts.
2. Export the shared private-route metadata from the admin layout and attach
